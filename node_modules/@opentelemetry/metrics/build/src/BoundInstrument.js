...
class BoundValueRecorder extends BaseBoundInstrument {
    constructor(labels, disabled, absolute, valueType, logger, aggregator) {
        super(labels, logger, disabled, valueType, aggregator);
        this._absolute = absolute;
    }
    record(value, correlationContext, spanContext) {
        if (this._absolute && value < 0) {
            this._logger.error(`Absolute ValueRecorder cannot contain negative values for $${Object.values(this._labels)}`);
            return;
        }
        this.update(value);
    }
